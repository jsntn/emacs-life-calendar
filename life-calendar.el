	...
	(let ((data nil))
	  ...
	  (with-temp-file life-calendar-data-file
	    ...
	    (dolist (entry (reverse data))  ; Reverse to maintain order
	      (let ((var (car entry))
		    (val (cadr entry)))
		(insert (format "(setq %s" var))
		;; Special formatting for life-calendar-chapters
		(if (and (eq var 'life-calendar-chapters)
			 (listp val))
		    (progn
		      ;; Sort chapters by date (earliest to latest)
		      (setq val (sort (copy-sequence val)
				      (lambda (a b)
					(string< (car a) (car b)))))
			...
			(dolist (chapter val)
			  (unless first
			    (insert "\n    "))
			  (setq first nil)
			  (insert (format "(%S . %S)" (car chapter) (cdr chapter))))
			(insert "))\n")))
